...
public class Cons extends Node {
    ...
    public boolean isRational() {
    	if(car.toString().trim().equals("rational"))
    		return true;
    	return false;
    }
    ...
    public Node getNumerator() {
    	if(car.toString().trim().equals("rational")) {
    		if(cdr.getCdr().getCar().isPositive())
    			return cdr.getCar();
    		else if(cdr.getCdr().getCar().isNegative())
    			return new IntLit(-cdr.getCar().getNodeInt());
    	}
    	return Nil.getInstance();
    }
    public Node getDenominator() {
    	if(car.toString().trim().equals("rational")) {
    		return new IntLit(Math.abs(cdr.getCdr().getCar().getNodeInt()));
    	}
    	return Nil.getInstance();
    }
    ...
    public boolean isPositive(){
    	if(car.toString().trim().equals("rational")) {
    		if(cdr.getCar().getNodeInt()*cdr.getCdr().getCar().getNodeInt()>0) {
    			return true;
    		}
    	}
    	return false;
